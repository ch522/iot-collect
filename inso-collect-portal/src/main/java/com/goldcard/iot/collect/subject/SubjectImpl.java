...
public class SubjectImpl implements ISubject {
    ...
    public void register(AbstractObserver observer) {
        if (!hasObserver(observer.sessionId())) {
            iotCacheManager.put(observer.sessionId(), observer);
        }
    }
    ...
    public void remove(String sessionId) {
        if (hasObserver(sessionId)) {
            this.iotCacheManager.remove(sessionId);
        }
    }

    @Override
    public void send(String sessionId) {
        AbstractObserver observer = this.iotCacheManager.get(sessionId);
        if (null != observer) {
            observer.send();
        }
    }
    ...
    public Boolean hasObserver(String sessionId) {
        return iotCacheManager.hasKey(sessionId);
    }
}
